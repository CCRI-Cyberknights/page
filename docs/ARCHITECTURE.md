...
- Styling: All files now use idiomatic Tailwind CSS with JIT configuration and `@layer` directives. Main SPA (`index.html`) keeps CSS variables in @layer base for JavaScript compatibility, but HTML uses Tailwind utilities. Zero !important declarations across entire codebase. See TAILWIND-IDIOMS.md for complete details.
